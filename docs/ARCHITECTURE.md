...
|---|-------|------|-----------|----------------------------|
| 1 | `clean` | Choix des colonnes (table de décision), normalisation ordinale, **imputation médiane/moyenne/zéro/KNN/itérative**, **aberrants IQR ou z-score** (borner/supprimer), exclusion univariée, doublons, **diagnostic data-quality par colonne**. **Stratégies par colonne** (`column_strategies`) : le tableau qualité est actionnable — imputation et traitement d'aberrants réglables colonne par colonne (contrôle `strategy_table` → `StrategyTable.jsx`), avec recommandation déduite du diagnostic (`_column_strategy`) et bouton « Appliquer les recos ». EDA détaillée (valeurs catégorielles, lignes extrêmes). L'étape exploratoire reste full-frame (inspection) ; côté modélisation, imputation et bornes sont **refit sur le train seul** par `separate` (`run(stats=False)` + `CleanPreprocessor`, overrides par colonne compris). | idem | idem |
| 2 | `transform` | Features dérivées (**métier + interactions produits/rapports + binning quantile/uniforme**, base et bornes fit train-only via `feature_engineering.py`), asymétrie, encodage **ordinal ordonné** / One-Hot nominales, scaling. **EDA univariée + bivariée par type**. | idem | bivariée vs cible désactivée (pas de cible) |
...
| 4 | `separate` | Split train/test stratifié + contrôle de fuite ; **chemin anti-fuite complet** : split d'abord sur le frame nettoyé STRUCTURELLEMENT, puis **statistiques du Nettoyage refit train-only** (imputation, bornes d'aberrants — `CleanPreprocessor` dans `stages/clean.py`) et **préprocesseur features** ajusté sur le train seul (`preprocessing.py`) ; les deux objets sont réutilisés tels quels par serving/export (aucun refit). Les lignes de test ne sont jamais supprimées (`*_remove` et `drop_rows` s'appliquent au train seul). | X/y train/test | `X_full` (pas de split) |
